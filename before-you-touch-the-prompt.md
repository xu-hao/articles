...
There's a better runbook, and it fits in the time you'd have spent arguing in Slack and saves your the embarrassment when it becomes a recurring issue. It's Benchmark-Driven Development (BDD) applied to incidents: **the reproduction of a semantic bug is a fraction, not a red test.** This is nothing surprising about BDD. It's the good-ole data-driven approach. Your prompt is your machine learning model. Why would you change it without eval? If you haven't build one and your prompt is in production, don't panic. You will see by the end of this article that you can build one incrementally with this approach.
...
It may be extremely enticing to retry the prompt in a playground and mark the error as a fluke. But pasting the input into a playground and running it once tells you *nothing*.

If the true failure rate on this input is 40%, a single retry shows the right answer six times out of ten, and "works for me" plus a closed ticket is exactly how this incident becomes a recurring series.

Skip the playground. Go to step 3.

## Step 3: Turn the incident into a case
...
Note the control group. The incident case tells you if you've reproduced the bug; the cases that currently work will tell you later whether your fix broke anything. Both matter.
...
- **`N ERRORED`** - model never answered wrong; your harness is failing (rate limits, timeouts, expired keys). Page the platform team, not the prompt engineer.
...
Before you write prompt v2, write the acceptance criteria, or look up SLA, if there is one, (if there is none, that's another story) while you're still objective:
...
Check it against the bar from step 5: incident case fixed outright, control cases held or improved, cost 3× , which is inside the ceiling. Ship it. If it hadn't cleared the bar, you'd iterate v3 against the same table instead of arguing about whose three manual tries felt better.
...
Then close the ticket the way this methodology teaches you to close every ticket with a reproducible fraction:
...
Next time production breaks at 4:40 on a Thursday, you'll have a fraction by 5:00, a measured fix by 5:30, and a sentinel by 5:31 — and you will never again close a semantic bug with the words "cannot reproduce." Over time, you will have built a proper eval dataset based on real usage.
